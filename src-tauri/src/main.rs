...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .invoke_handler(tauri::generate_handler![cmd_init, cmd_select_mirror])
        .setup(move |app| {
            ...
            // Model init is triggered by frontend via cmd_init after JS is ready
...
/// Tauri command: called by frontend after JS is ready, triggers model init.
#[tauri::command]
fn cmd_init(app: AppHandle, data: tauri::State<'_, Arc<AppData>>) {
    let handle = app.clone();
    let data_clone = data.inner().clone();
    std::thread::spawn(move || {
        init_model(handle, data_clone);
    });
}

/// Check model status: if ready → load; if missing → show mirror selection.
fn init_model(handle: AppHandle, data: Arc<AppData>) {
    ...
    // Model missing or incomplete, always let user choose download source
    log::info!("Model not ready, asking user to select download source");
    let _ = handle.emit("need-select-mirror", ());
    show_overlay(&handle);
}

/// Payload sent to frontend on download failure, carrying the failed mirror name.
...
/// Download model from the given mirror, then load it.
fn download_and_load(
    ...
    let url = model_downloader::get_download_url(mirror);
    ...
    if let Err(e) = result {
        ...
        let _ = handle.emit("download-error", DownloadError {
            message: e,
    ...
    if !whisper_engine::model_is_ready() {
        ...
        let _ = handle.emit("download-error", DownloadError {
            message: "下载的模型文件无效，请重试".to_string(),
            mirror: mirror.to_string(),
        });
        return;
    }
...
/// Tauri command: user selects download mirror ("cn" or "global"), save and start download.
#[tauri::command]
fn cmd_select_mirror(
    app: AppHandle,
    data: tauri::State<'_, Arc<AppData>>,
    mirror: String,
) -> Result<(), String> {
    log::info!("User selected mirror: {}", mirror);

    // Persist choice to config so next launch won't ask again
    let mut cfg = config::load_config();
    cfg.general.mirror = mirror.clone();
    ...
    std::thread::spawn(move || {
        ...
        download_and_load(&handle, &data_clone, &cache_path, &mirror);
    });
